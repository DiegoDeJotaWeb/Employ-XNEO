...
class Tarefa extends Database
{
    ...
    public function criarTarefa($userId)
    {
        ...
        $query = $database->prepare("insert into tbl_tarefa(descricao_tarefa,status_tarefa,user_id) values (:descricao,:status,:userId)");
    ...
    public function editarTarefa()
    {
        ...
        $database->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $query = $database->prepare("update tbl_tarefa set descricao_tarefa = :descricao where id_tarefa = $id");
        $query->bindValue(":descricao", "$descricao");
        $query->execute();
        header("Location: index.php");
    }
